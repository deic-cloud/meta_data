...
**Tags and metadata values of shared files live with the owner.** Tags work the same way as values below: for a file in a share received from another node, tags are read from the owner's node whatever is local (`ApiController::getFileTags` → `getRemoteFileTags`), and adding/removing a tag (`TagService::addFileTag/removeFileTag` with the user) is done on the owner's node via `internal/filetag-by-token`, refused unless the share allows editing; the sidebar shows the reason and re-renders. **Several files at once:** with more than one file selected, the Files selection toolbar offers **Tags** (file action `meta_data:bulk-tags`, `execBatch`): the Metadata tab's tag list for all of them — a box is ticked when every selected file has the tag, half-ticked ("2 of 3") when some do; a click gives the tag to all, or removes it from all when all have it. Each change goes through the same `filetags` API as the tab (so it writes through for files shared from another node); per-file refusals are listed in the dialog.

Tags are edited in one place, the Metadata tab: `js/hide-core-tag-actions.js` switches off core's two other tag editors, "Manage tags" (file action `systemtags:bulk`, also in the multi-select toolbar) and "Add tags" (details-panel ⋯ menu, sidebar action `systemtags`), by making their `enabled()` answer false in `@nextcloud/files`' registries. They assign tags on the local node only, so for files shared from another node they would tag a copy nobody else sees. Existing tags still show in the file list.
